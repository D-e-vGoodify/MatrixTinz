    ...
    return (
        <div className="displayBox relative flex flex-wrap justify-center items-end w-full h-1/3 dark:text-white font-system abel:font-abel open:font-open barlow:font-barlow josefin:font-josefin montserrat:font-montserrat digital:font-digital googleSans:font-googleSans dotty:font-dotty orbitron:font-orbitron tiny:font-tiny px-4 overflow-hidden">
            ...
            <div className='relative w-full h-2/3 flex items-center justify-center'>
                <input className={`absolute w-full ${getDynamicTextSize()} outline-none text-right font-semibold origin-right transition-all duration-500 ease-in-out leading-normal ${showResult && !isNative ? 'bottom-1/10 scale-[60%] md:scale-50 px-4 cursor-default text-gray-600 dark:text-gray-300' : showResult && isNative ? 'bottom-3/10 scale-[60%] md:scale-50 px-4 cursor-default text-gray-600 dark:text-gray-300' : !showResult && !isNative ? 'bottom-[55%] md:bottom-5/10 px-2 scale-100' : 'bottom-[65%] md:bottom-5/10 px-2 scale-100'} caret-app-darker dark:caret-app-lighter selection:bg-app-dark dark:selection:bg-app-light selection:text-white dark:selection:text-black`} inputMode="none" onFocus={handleInputFocus} type="text" onKeyDown={isWeb ? disableKeyBoard : undefined} onKeyUp={handleCursorSnap} ref={inputRef} value={input || 0} onChange={(e) => setInput(e.target.value)} onClick={handleCursorSnap} readOnly={showResult} />
                <input onMouseDown={(e) => e.preventDefault()} className={`absolute right-0 text-5xl md:text-6xl dotty:text-7xl dotty:md:text-8xl orbitron:text-4xl orbitron:md:text-5xl googleSans:text-[39px] googleSans:md:text-[48px] outline-none text-right font-semibold origin-right transition-all duration-500 ease-in-out leading-normal ${showResult && !isNative ? 'w-full bottom-[55%] md:bottom-5/10 px-2 scale-100' : showResult && isNative ? 'w-full bottom-[65%] md:bottom-5/10 px-2 scale-100' : !showResult && !isNative ? 'w-3/2 bottom-1/10 scale-[60%] md:scale-50 px-4 text-gray-600 dark:text-gray-300' : 'w-3/2 bottom-3/10 scale-[60%] md:scale-50 px-4 text-gray-600 dark:text-gray-300'} border-none selection:bg-[#0F5C91] dark:selection:bg-[#85C4EE] selection:text-white dark:selection:text-black ${answer ? 'motion-translate-y-in-25' : ''} cursor-default`} value={answer} readOnly />
            </div>
        </div>
    )
}
